...
import org.apache.commons.fileupload.FileItem; //얘를 사용해서 
...
public class UploadSave extends HttpServletRequestWrapper {
  ...
  public static String saveFile(FileItem fileItem, String upDir) {
    ...
    // 파일 태그
    System.out.println("전송된 파일명: " + fileItem.getName());   /* 경로  + 파일명이다.*/
 
    // 폴더 구분자 추출 (ex)( c:/test/test.jpg)
    int idx = fileItem.getName().lastIndexOf("\\"); // 윈도우 기반
 
    if (idx == -1) { // 유닉스, 리눅스 기반                         (  \\이 없었다면 윈도우기반이 아닌 것이므로)
    ...
    try {
      ...
      if (uploadedFile.exists() == true) {
        for (int k = 0; true; k++) {                    /* true까지는 무한반복이라는 것임*/
